...
import { Search, ShieldCheck, FileText, Database, Activity, MessageSquare } from 'lucide-react';
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <div className="flex justify-between items-end mb-4 border-b border-gray-200 pb-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">{selectedPatient.name || selectedPatient.username} 的数字档案</h2>
          <p className="text-sm text-gray-500 mt-1">
            系统账号：<span className="font-mono text-gray-800 font-bold">{selectedPatient.username}</span> | 
            调阅时间：{new Date().toLocaleDateString('zh-CN')} {new Date().toLocaleTimeString('zh-CN', {hour: '2-digit', minute:'2-digit'})}
          </p>
        </div>
        <button onClick={onJumpToChat} className="flex items-center text-blue-600 bg-blue-50 px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-100 shadow-sm transition-colors">
          <MessageSquare className="w-4 h-4 mr-2"/> 私信沟通
        </button>
      </div>
