...
class NIPTPredictor:
    ...
    def retrain_xgboost(self, db):
        """
        🚀 完善逻辑：从数据库直接提取带有临床结论（标签）的样本，进行增量监督学习。
        解决 'Session' object is not subscriptable 的报错。
        """
        try:
            ...
            # 1. 从数据库拉取所有【已生成分析报告】的真实样本
            # XGBoost 是有监督学习，必须要有 X(特征) 和 y(标签)
            records = db.query(models.NIPTRecord).join(
                models.AIAnalysisResult, 
                models.NIPTRecord.id == models.AIAnalysisResult.nipt_record_id
            ).all()

            if len(records) < 50:
                return False, f"重训拦截：已标注样本仅 {len(records)} 例，不足以支撑 XGBoost 更新（需50例以上）"
            ...
            for r in records:
                ...
                row['bmi'] = float(user.bmi) if user and user.bmi else 22.0
                data_list.append(row)

                # 🚀 构建监督学习标签 (Ground Truth)
                # 真实医疗系统中，这里应该是随访穿刺的确诊结果。在我们的 SaaS 业务闭环中，
                # 假设 AIAnalysisResult 的 risk_level 判定为'高风险'即视为阳性标签(1)，否则为(0)
                label = 1 if r.analysis_result.risk_level == '高风险' else 0
                labels.append(label)

            # 3. 转化为 DataFrame 并进行特征工程
            ...
            joblib.dump(self.model_xgb, self.xgb_path) # 覆盖保存新模型
            
            # 6. 【核心】同步更新 SHAP 解释器，确保 XAI 归因图表不会报错
            self.explainer = shap.TreeExplainer(self.model_xgb) 
            
            return True, f"XGBoost 专家引擎演进成功，已深度学习 {len(records)} 例带标注临床样本"

        except Exception as e:
            # 返回具体错误信息，而不是直接抛出 500
            import traceback
            print(traceback.format_exc()) # 在控制台打印详细堆栈方便排查
    ...
    def retrain_iforest(self, db):
        ...
        try:
            ...
            records = db.query(models.NIPTRecord).join(
                ...
                models.AIAnalysisResult.risk_level == '低风险'
            ).all()
